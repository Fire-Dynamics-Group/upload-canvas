...
// closest approach to the drawn boundary (the engineer can override it);
// boundaryDistance is null when no boundary is drawn. This seed is not the
// BR 187 perpendicular measurement — enclosing-rectangle uses one distance
// per elevation.
export function bre135ElevationsFromWall(wallPoints, boundaryPoints, sampleStep = 0.5) {
    ...
    return faces.map((face, i) => {
        ...
        if (hasBoundary && width > 0) {
            ...
            let min = Infinity
            const consider = (d) => {
                // BRE 135 takes one enclosing-rectangle distance per elevation.
                // Seed from closest approach, not the BR 187 perpendicular, so a
                // side face still gets a default the engineer can override.
                const dist = pointToPolylineDistance(pointAtDistanceAlong(face.points, d), boundaryPoints)
                if (Number.isFinite(dist)) min = Math.min(min, dist)
            }
