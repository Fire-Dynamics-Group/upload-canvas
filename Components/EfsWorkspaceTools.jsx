...
export default function EfsWorkspaceTools({ onChangeMode }) {
    ...
    // Keep the drawing tools collapsible while retaining Kevin's calculation
    // panel behavior: visible by default, with a compact/expanded width toggle.
    ...
    return <>
        <aside className={`efs-drawing-panel ${drawingOpen ? '' : 'is-collapsed'}`} aria-label="Drawing tools">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">
                {tool === 'efsBay' ? 'Set column spacing on the right, then click a wall segment. Column distances update after each click.' : tool === 'polyline' ? 'Click to add points. Press Enter to finish the line.' : 'Select a drawing point to edit it. Hold Space and drag to pan; use Ctrl + wheel or trackpad scroll to zoom.'}
            </p>
